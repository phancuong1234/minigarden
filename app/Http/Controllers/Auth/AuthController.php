       ...
       // die($data);
        if(count($data)>0){
                    foreach ($data as $value) {
                        Session::put('account', array('username' => $value->Username,'fullname' => $value->Fullname,'email' => $value->Email,'password' => $value->Password,'MacAddress'=>$value->MacAddress,'status'=>$value->Status,'numberplant'=>$value->Number_Bonsai));
                    }
